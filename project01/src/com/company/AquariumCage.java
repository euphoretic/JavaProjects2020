...
public class AquariumCage extends Cage {
    public AquariumCage(int numberId){
        super(numberId,"AquariumCage");
        // Logger.post("INFO", "Создан новый аквариум");
    }


    @Override
    public void putAnimal(Animal animal) {
        if (animal.getType() == "Waterfowl") {
            super.putAnimal(animal);
            // Logger.post("INFO", "В аквариум добавлено новое водоплавающее животное");
        } else {
                    ...
                    if (!key.isEmpty() && !value.isEmpty()) {
                        switch (key) {
                            ...
                            case "Animal" -> {
                                ...
                                this.putAnimal(new Waterfowl(numArrParam[0], numArrParam[1], this.getType()));
                                Result = true;
                            }
                        }
                    }
