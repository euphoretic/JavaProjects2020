...
public class OpenCage extends Cage{
    public OpenCage(int numberId){
        super(numberId,"OpenCage");
        // Logger.post("INFO","Создан новый открытый вольер");
    }

    @Override
    public void putAnimal(Animal animal) {
        if (animal.getType() == "Ungulate") {
            super.putAnimal(animal);
            // Logger.post("INFO","В открытый вольер добавлено новое копытное животное");
        } else {
                    ...
                    if (!key.isEmpty() && !value.isEmpty()) {
                        switch (key) {
                            ...
                            case "Animal" -> {
                                ...
                                this.putAnimal(new Ungulate(numArrParam[0], numArrParam[1], this.getType()));
                                Result = true;
                            }
                        }
                    }
